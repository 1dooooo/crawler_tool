...
get_browser = Browser('chrome', user_agent='Mozilla/5.0 (iPhone; U; CPU like Mac OS X; en)')
...
def login(browser,username,password):
    ...
    time.sleep(2)
    browser.fill("loginfmt",str(username))
    browser.find_by_id('idSIButton9').first.click()
    time.sleep(5)
    browser.fill("passwd",str(passwd))
    browser.find_by_id('idSIButton9').first.click()
    time.sleep(60)
...
while ok<=1:
    ...
    while status == 0:
        ...
        status = register(go_browser,username,password)

        time.sleep(5)
    go_browser.quit()
    status = login(get_browser,username,password)
    get_browser.quit()
    ok+=1
